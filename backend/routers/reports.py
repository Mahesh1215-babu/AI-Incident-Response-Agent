...
async def get_dashboard_reports(current_user: dict = Depends(get_current_user_from_token)):
    ...
    if total_incidents == 0:
        ...
        avg_res_time = 14.5
        recent_activity = [
            {"id": "mock-1", "title": "PostgreSQL database client pool connection leak", "severity": "High", "status": "Resolved", "created_at": (datetime.utcnow() - timedelta(hours=2)).isoformat(), "created_by": "engineer@incidentagent.com"},
            {"id": "mock-2", "title": "Kubernetes pod node CPU threshold alert", "severity": "Medium", "status": "Open", "created_at": (datetime.utcnow() - timedelta(hours=4)).isoformat(), "created_by": "admin@incidentagent.com"},
            {"id": "mock-3", "title": "Auth Gateway JWT key verification exception", "severity": "Critical", "status": "Resolved", "created_at": (datetime.utcnow() - timedelta(days=1)).isoformat(), "created_by": "engineer@incidentagent.com"},
            {"id": "mock-4", "title": "Redis Cache OOM volatile-lru purge fault", "severity": "Critical", "status": "Resolved", "created_at": (datetime.utcnow() - timedelta(days=2)).isoformat(), "created_by": "engineer@incidentagent.com"}
        ]
