...
const AdminPanel = () => {
  ...
  const handleTestWebhook = async () => {
    try {
      ...
      const res = await reportsAPI.testWebhook({ event: "test", timestamp: new Date().toISOString() });
      setWebhookStatus(res.message);
    } catch (err) {
      setWebhookStatus("Failed to send webhook.");
    } finally {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* Webhook Integrations (Task 8) */}
      <div className="glass-card space-y-4">
        <div>
          <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <Activity className="w-5 h-5 text-indigo-500" />
            <span>Outbound Integrations (Webhook)</span>
          </h3>
          <p className="text-xs text-slate-400 mt-0.5">Test PagerDuty / Slack outbound webhook delivery</p>
        </div>
        <div className="p-4 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center justify-between">
          <div>
            <h4 className="text-sm font-semibold text-white">Test Webhook Stub</h4>
            <p className="text-xs text-slate-400">POST /integrations/webhook-test</p>
          </div>
          <div className="flex items-center gap-4">
            {webhookStatus && <span className="text-emerald-400 text-xs font-semibold animate-pulse">{webhookStatus}</span>}
            <button
            ...
            >
              {triggeringWebhook ? "Sending..." : "Trigger Webhook"}
            </button>
          </div>
        </div>
      </div>
